...
try {
    ...
    Write-Host " FASE 8E.9G - ROLLBACK OPERACIONAL NA CENTRAL" -ForegroundColor Cyan
    Write-Host " HISTORICO DE VERSOES, ROLLBACK VISUAL E AUDITORIA D1" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    ...
    if (-not (Test-Path $testPublish)) { Fail "Teste E2E ausente." }
    ...
    $audit = Join-Path $root ".auditoria\Online\8E9G_$stamp"
    New-Item -ItemType Directory -Force -Path $audit | Out-Null

    git bundle create (Join-Path $audit "PRE_8E9G.bundle") --all
    if ($LASTEXITCODE -ne 0) { Fail "Checkpoint Git falhou." }

    Copy-Item $worker (Join-Path $audit "worker.8E9G.js") -Force
    Copy-Item $central (Join-Path $audit "central.8E9G.html") -Force
    ...
    Write-Host " FASE 8E.9G CONCLUIDA COM SUCESSO!" -ForegroundColor Green
    Write-Host " ROLLBACK OPERACIONAL E HISTORICO HOMOLOGADOS NA CENTRAL" -ForegroundColor Green
    ...
    Write-Host " FALHA NA EXECUCAO DA FASE 8E.9G" -ForegroundColor Red
